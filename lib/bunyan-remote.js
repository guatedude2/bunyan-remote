...
function isIPAllowed(ipList, ip) {
...
function BunyanRemote(options) {
  ...
  var io = socketIO(options.server || options.port || 3232,  {
    serveClient: false
  });
  ...
  io.use(function (socket, next) {
    var clientIp = ipaddr.process(socket.handshake.address).toString();

    if (!options.allowedIPs || isIPAllowed(options.allowedIPs, clientIp)) return next();
    socket.disconnect();
